...
const initialCosts: CostItem[] = [
  ...
  {
    id: 'electricidad_amasadora',
    category: 'Masa',
    label: 'Electricidad amasadora',
    price: 0.0358,
    quantity: 1,
    vat: 10,
    unitType: 'unidad',
  ...
  {
    id: 'papel_kraft',
    category: 'Envasado y Etiquetado',
    label: 'Papel kraft base',
    price: 0.11,
    quantity: 1,
    vat: 21,
    unitType: 'unidad',
  ...
  },
  {
    id: 'folio_direccion',
    category: 'Envasado y Etiquetado',
    label: 'Folio etiqueta dirección',
    price: 0.0043,
    quantity: 1,
    vat: 21,
    unitType: 'unidad',
...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/empanadas')
      ...
      .then((list) => {
        ...
        if (nameParam) {
          const emp = list.find((e: Empanada) => e.name === nameParam)
  ...
  const handleExcelUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setShowTotals(false)
      setDirty(true)
      toast.success('Empanada cargada desde Excel', {
  ...
  const saveEmpanada = async (overrideName?: string, skipConfirm?: boolean) => {
    ...
    if (
      !skipConfirm &&
      exists &&
      !confirm('Ya existe una empanada con ese nombre. ¿Desea sobrescribirla?')
    ) {
      return
    }
  ...
  const loadEmpanada = (emp: Empanada) => {
    ...
    setDirty(false)
    toast.success('Empanada cargada', {
      style: { background: '#16a34a', color: '#fff' },
    })
  }
  ...
  return (
    <div className='p-4 sm:p-6 mt-6 font-sans max-w-full lg:max-w-screen-lg mx-auto bg-white rounded-lg shadow-lg'>
      <h1 className='text-2xl font-bold mb-4'>
        {name && `${name} - `}Calculadora de Costes de Empanadas
      </h1>
                ...
                <td>
                  {newEntries[cat]?.productName && (
                    <input
                      ...
                      step={(() => {
                        const prod = products.find(
                          (p) => p.name === newEntries[cat]?.productName
                        )
                        return prod &&
                          (prod.unitType === 'envase' ||
                            prod.unitType === 'unidad')
                          ? 1
                          : 0.0001
                      })()}
